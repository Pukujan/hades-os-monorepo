...
export function createSlackConnectionRepository({ storage = "memory", supabaseClient, tableName = "hades_slack_connections", crypto = null } = {}) {
  ...
  async function saveToken({ userId, tenantId, token }) {
    await hydrate();
    if (!crypto || typeof crypto.encrypt !== "function") {
      throw Object.assign(
        new Error("Crypto dependency is required to store Slack tokens"),
        { code: "missing_crypto" }
      );
    }
  ...
  async function findBySlackUserId({ slackUserId }) {
    await hydrate();
    for (const record of connections.values()) {
      if (record.slack_user_id === slackUserId) {
        return record;
      }
    }
    return null;
  ...
  async function findRuntimeTokenByUserId({ userId, tenantId }) {
    ...
    if (!crypto || typeof crypto.decrypt !== "function") {
      throw Object.assign(
        new Error("Crypto dependency is required to decrypt Slack tokens"),
        { code: "missing_crypto" }
      );
    }
  ...
  return { saveToken, findPublicByUser, findBySlackUserId, findRuntimeTokenByUserId, findRecord, delete: deleteRecord };
}
